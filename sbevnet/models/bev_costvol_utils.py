...
# def get_grid_one(cam_conf, img_h, img_w, n_hmap, xmax, xmin, ymax, ymin, max_disp, camera_ext_x, camera_ext_y, rotation_matrix=None):
#     remap_normed_inv = np.zeros((n_hmap, n_hmap, 2))
#     assert len(cam_conf) == 4 
#     f, cx, cy, tx = [float(x) for x in cam_conf]
    
#     # if rotation_matrix is None:
#     #     rotation_matrix = np.eye(3)
    
#     assert rotation_matrix is not None, "rotation_matrix is None"

#     key = str(f) + str(cx) + str(cy) + str(tx) + str(rotation_matrix.tobytes())
...
#                 point_rotated = rotation_matrix @ point_pc
